    ...
    while (running && !bee->dead) {
        ...
        if (bee->outside) {
            ...
            if (hive->bees_in_hive < hive->max_bees_in_hive) {
                ...
                hive->bees_in_hive++;
                bee->outside = false;
                bee->Ti = (rand() % 5 + 5) * 1000; // Nowy czas pobytu w ulu
            } else {
        ...
        // Aktualizacja wieku i liczby wizyt
        bee->visits++;
        bee->age++;

        // Sprawdzenie, czy pszczoła umiera
        if (bee->visits > 3) {
        ...
        // Zwalnianie globalnego semafora
        sb.sem_op = 1;
        if (semop(sem_id, &sb, 1) == -1) {
            perror("Błąd podczas zwalniania semafora w bee_process");
            exit(EXIT_FAILURE);
